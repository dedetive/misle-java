		...
		if (gameState == GameState.PLAYING) {
			...
			if (!player.attr.isDead()) {
				...
				if (willMovePlayer[0] != 0 || willMovePlayer[1] != 0) {
					if (!Physics.isPixelOccupied((player.pos.getX() + willMovePlayer[0]), player.pos.getY(), player.attr.getWidth(), player.attr.getHeight(), range, 12, Physics.ObjectType.PLAYER)) {
						Physics.movePlayer(willMovePlayer[0], 0);
					}
					if (!Physics.isPixelOccupied(player.pos.getX(), (player.pos.getY() + willMovePlayer[1]), player.attr.getWidth(), player.attr.getHeight(), range, 12, Physics.ObjectType.PLAYER)) {
